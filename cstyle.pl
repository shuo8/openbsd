...
#		\B	non-word boundary
#
# This version has been modified to reflect OpenBSD KNF (style.9) for use with
# COMP3301 at the University of Queensland.
#
...
"usage: cstyle [-chpvCP] [-o constructs] file ...
	-c	disable continuation indentation checks inside functions
	-h	perform heuristic checks that are sometimes wrong
	-p	disable some of the more picky checks
	-v	verbose
...
line: while (<$filehandle>) {
	...
	if (/\b(sizeof) \(/) {
		err("space between sizeof and paren");
	}
	...
	if (/^(\w+(\s|\*)+)+\w+\(.*\)\s*[\{](\s|\cA)*$/) {
		err("return type and open curly of function not on separate line");
	}
	...
	if ($picky) {
		...
		if (/\b$typename\s*\*\s/o &&
			...
			err("unary * followed by space, or pointer * alignment (should be 'char *foo')");
		}
	}
